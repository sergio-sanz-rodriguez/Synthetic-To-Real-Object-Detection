...
def predict_and_save(model_1, model_2, image_path, output_path_txt, device):
    """
    Predict bounding boxes using a custom model and save them in YOLO format.
    
    Args:
        model_1/model_2: object detection models.
    ...
    with torch.no_grad():
        ...
        # If the model is unable to detect RoIs, make predictions using the second model
        else:
            pred = model_2([x, ])[0]
            if pred["boxes"].nelement() > 0:
                pred = prune_predictions(pred)
            else:
                print(f"No prediction for {image_path}")
...
def main():
    ...
    parser.add_argument("--model2", type=str, required=True, help="Path to the second model file")
    ...
    model_1 = load_model(model_1, Path(args.model1).parent, Path(args.model1).name, device)
    model_2 = load_model(model_2, Path(args.model2).parent, Path(args.model2).name, device)
    ...
    for img_path in images_dir.glob('*'):
        ...
        predict_and_save(model_1, model_2, img_path, output_path_txt, device)
